...
| **クイックタスク** | メッセージのアクションメニューに「my」ボタンを追加。マイチャット/現チャット × URL/メッセージの4モード対応。期限デフォルトを日数で指定可能（デフォルト3日後）。メッセージ本文はCW純正の「引用」機能経由で取得するため送信者アイコン付きで表示され、メンション・絵文字も崩れない。共通APIキーを設定している場合はタスクAPI経由で登録するため画面遷移なし（未設定時はDOM操作経路にフォールバック） |
...
| **クイック削除** | 自分のメッセージの右上に×ボタンを表示し、ワンクリック（確認ダイアログあり）で削除。共通APIキー必須、デフォルトOFF |
